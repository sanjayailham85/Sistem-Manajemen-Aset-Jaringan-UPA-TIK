...
} from "../../services/cctvControllerService";
import usePagination from "../../utils/usePagination";
import usePermission from "../../utils/usePermission";
import CCTVControllerModal from "../../components/digital/cctv/CCTVControllerModal";
import {
  notifyCreate,
  notifyUpdate,
  notifyDelete,
...
const ControllerTable = () => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await deleteCCTVController(id);
      await refresh();
      notifyDelete("");
    } catch (err) {
      const code = err?.response?.data?.code;
      if (code === "CCTV_NOT_EMPTY") {
        notifyDeleteError("");
      } else {
        notifyError();
      }
      ...
      <table className="w-full text-sm">
        <thead className="bg-gray-100">
          <tr>
            ...
            <th className="px-4 py-2 text-left">Total CCTV</th>
            {(canUpdate || canDelete) && (
        ...
        <tbody>
          {data.map((item) => (
            <tr
              key={item.id}
              onClick={() =>
                navigate(`/cctvMerk/${merkId}/controller/${item.id}`)
              }
              className="border-t hover:bg-gray-50 cursor-pointer"
            >
              <td className="px-4 py-2">{item.ip}</td>
              <td className="px-4 py-2">{item.totalCCTV}</td>
