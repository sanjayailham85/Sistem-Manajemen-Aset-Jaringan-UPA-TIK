...
} from "../../services/accessPointControllerService";
import usePagination from "../../utils/usePagination";
import usePermission from "../../utils/usePermission";
import AccessPointControllerModal from "../../components/digital/accessPoint/AccessPointControllerModal";
import {
  notifyCreate,
  notifyUpdate,
  notifyDelete,
...
const ControllerTable = () => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await deleteAccessPointController(id);
      await refresh();
      notifyDelete("");
    } catch (err) {
      const code = err?.response?.data?.code;
      if (code === "AP_NOT_EMPTY") {
        notifyDeleteError("");
      } else {
        notifyError();
      }
      ...
      <table className="w-full text-sm">
        <thead className="bg-gray-100">
          <tr>
            ...
            <th className="px-4 py-2 text-left">Total Access Point</th>
            {(canUpdate || canDelete) && (
        ...
        <tbody>
          {data.map((item) => (
            <tr
              key={item.id}
              onClick={() =>
                navigate(`/accessPointMerk/${merkId}/controller/${item.id}`)
              }
              className="border-t hover:bg-gray-50 cursor-pointer"
            >
              <td className="px-4 py-2">{item.ip}</td>
              <td className="px-4 py-2">{item.totalAccessPoint}</td>
