...
import {
  ...
  updateMerk,
  deleteMerk,
} from "../../services/merkService";
import usePagination from "../../utils/usePagination";
import usePermission from "../../utils/usePermission";
import MerkModal from "../../components/option/MerkModal";
import {
  notifyCreate,
  notifyUpdate,
  notifyDelete,
...
const MerkTable = () => {
  ...
  const handleAdd = async (data) => {
    try {
      await createMerk(data);
      await refresh();
      setOpenModal(false);
      notifyCreate("Merk");
    } catch (err) {
      console.error(err);
      notifyError("Gagal menambah Merk");
    }
  };

  const handleUpdate = async (data) => {
    try {
      await updateMerk(selected.id, data);
      await refresh();
      setSelected(null);
      setOpenModal(false);
      notifyUpdate("Merk");
  ...
  const handleDelete = async (id) => {
    try {
      ...
      console.error(err);
      notifyError("Gagal menghapus Merk");
    }
  };
